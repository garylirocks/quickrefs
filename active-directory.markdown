...
- **Organization Unit (OU)**
  - Objects are placed in OUs, forming a hierarchy
...
- **Domain Controllers (DC)**
  ...
  - Only one DC will hold the FSMO roles (Flexible Single Master Operation), like
    - RID Master
    - PDC Master
...
- **Domain local**
  - Membership: users, computers, and Global/Universal groups from any domain in the forest
  - Usage: permissions in the same domain
  - Replication: same domain
- **Global**
  - Membership: users, computers, other Global gropus in the same domain
  - Usage: permssions in any domain in the forest
  - Replication: same domain
- **Universal**
  - Membership: users, computers, other Global/Universal from any domain
  - Usage: permssions in any domain in the forest
  - Replication: GC servers across the forest

### Password policies

- **Domain level**
  - In "**Default Domain Policy**" GPO (*Computer Configuration > Policies > Windows Settings > Security Settings > Account Policies > Password Policy*)
- **Fine-Grained Password Policy** (FGPP)
  - Configure with `Active Directory Administrative Center` (ADAC)
  - In "Domain > System > Password Settings Container"
  - Create a setting and apply to specific users or groups
  - Overrides the domain level policy
  - If multiple FGPPs apply to a user, lower precedence number means higher priority
